...
function setupCanvas(lineChartData){
    ...
    chartGroup.selectAll('.line-series').data(lineChartData.series).enter()
              ...
              .attr('d',d=>lineGen(d.values)).style('fill','none').style('stroke',d=>d.color);

    const scatters = this_svg.selectAll('.scatter').data(lineChartData).enter()
                             .append('circle')
                             .attr('class','scatter')
...
d3.csv('line_chart_container/data/movies.csv',type).then(
    res => {
        ready(res);
    }
);